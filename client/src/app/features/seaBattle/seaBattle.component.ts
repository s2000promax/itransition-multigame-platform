...
export class SeaBattleComponent implements OnInit, OnDestroy {
            ...
            this.gameServerService.gameState$.subscribe((gameState) => {
                if (gameState) {
                    ...
                    if (gameState.players[0].availableMoves?.length) {
                        this.availableMoves =
                            gameState.players[0].availableMoves;
                    }
                    ...
                    if (gameState?.players[0]?.movesMyBoardPlayer) {
                        this.updateFieldAfterMover(
                            gameState?.players[0]?.movesMyBoardPlayer,
                            true,
                        );
                    }
                    ...
                    if (gameState?.players[0]?.movesRivalBoardPlayer) {
                        this.updateFieldAfterMover(
                            gameState?.players[0]?.movesRivalBoardPlayer,
                            false,
                        );
                    }
    ...
    private initializeCanvas() {
        ...
        this.offsetX =
            (this.game.view.width -
                this.cols * (this.spriteSize + this.padding) +
                this.padding) /
            2;

        this.totalFieldsHeight =
            2 * this.rows * (this.spriteSize + this.padding) +
            this.fieldsOffsetY;
        this.offsetYBase =
            (this.game.view.height - this.totalFieldsHeight + this.padding) / 2;
    }
    ...
    createSprites() {
        ...
        const offsetY =
            (this.game.view.height -
                this.rows * (this.spriteSize + this.padding) +
                this.padding) /
            2;

        const fieldsOffsetY = 50;
        const totalFieldsHeight =
            2 * this.rows * (this.spriteSize + this.padding) + fieldsOffsetY;
    ...
    private createMyField(board: BoardT) {
        for (let row = 0; row < this.rows; row++) {
            for (let col = 0; col < this.cols; col++) {
                if (board[row][col] === 'S') {
                    const someCell = new CellSpriteModel(
                        row,
                        col,
                        (row, col) => {},
                    );
