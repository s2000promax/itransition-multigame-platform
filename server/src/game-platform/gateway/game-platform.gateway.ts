    ...
    async makeMove(client: Socket, dto: MoveDto) {
        ...
        if (session.gameInstance instanceof SeaBattleGame) {
            ...
            if (gameState.status === 'win') {
                ...
                const loserSocket = this.server.to(gameState.players.find(player => player.id !== gameState.currentPlayer.id).id);
                ...
                for (const connection of connections) {
                    const gameStateResponse = {
                        ...gameState,
                        players: gameState.players.filter(player => player.id == connection),
                    }
