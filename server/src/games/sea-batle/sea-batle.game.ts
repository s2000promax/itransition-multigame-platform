...
export class SeaBattleGame {
    ...
    constructor(players: Player[]) {
        ...
        this.currentPlayer = this.players[Math.floor(Math.random() * players.length)];
        ...
        this.players = players.map((player, index) => {
            return {
                ...
                rivalName: index === 0 ? this.players[1].username : this.players[0].username,
                board: index === 0 ? this.boardPlayer1 : this.boardPlayer2,
                availableMoves: index === 0 ? this.availableMovesBoardPlayer1 : this.availableMovesBoardPlayer2,
        ...
        for (const ship of this.ships) {
            for (let i = 0; i < ship.count; i++) {
                while (true) {
                    ...
                    if (this.isValidPlacement(board, row, col, direction, ship.size)) {
    ...
    makeMove(row: number, col: number): GameState {
        const currentUserIndex = this.players.findIndex(player => player.id === this.currentPlayer.id);
        this.players[currentUserIndex].availableMoves[row][col] = false;

        const opponentIndex = (currentUserIndex === 0) ? 1 : 0;
        const symbol = this.players[opponentIndex].board[row][col] === 'S' ? 'H' : 'M'
        if (symbol === 'H') {
    ...
    private isValidPlacement(board: BoardT, row: number, col: number, direction: string, size: number): boolean {
        for (let i = 0; i < size; i++) {
            if (direction === 'horizontal') {
                if (col + i >= 10 || board[row][col + i] === 'S' || !this.isSafe(board, row, col + i)) {
                    return false;
                }
            } else {
                if (row + i >= 10 || board[row + i][col] === 'S' || !this.isSafe(board, row + i, col)) {
                    return false;
                }
            }
        }
    ...
    private isSafe(board: BoardT, row: number, col: number): boolean {
        ...
        for (const [dx, dy] of directions) {
            ...
            if (newRow >= 0 && newRow < 10 && newCol >= 0 && newCol < 10 && board[newRow][newCol] === 'S') {
                return false;
            }
        }
    ...
    private placeShip(board: BoardT, row: number, col: number, direction: string, size: number): void {
        for (let i = 0; i < size; i++) {
            if (direction === 'horizontal') {
                board[row][col + i] = 'S';
            } else {
